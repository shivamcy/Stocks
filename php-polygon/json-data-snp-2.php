...
try {
    ...
    $snp500_data = json_decode(file_get_contents('snp500.json'), true);

    // Convert S&P 500 data to an array of symbols for easy lookup
    $snp500_symbols = array_column($snp500_data, 'Symbol');

    $results = $query_response->fetchAll();
    foreach ($results as $key => $row) {
        // Check if the ticker is in the S&P 500 list
        if (!in_array($row['ticker'], $snp500_symbols)) {
            unset($results[$key]); // Remove the ticker if it's not in the S&P 500
        }
    }
    $json = json_encode(array_values($results)); // Re-index the array and encode to JSON
    echo $json;
